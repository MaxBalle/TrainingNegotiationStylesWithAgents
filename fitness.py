...
def accommodating(outcome: tuple[int, float, float, int], negotiation_scenario_perspective: ScenarioPerspective) -> float:
    return __calc_fairness(outcome, time_weight=-0.025, partner_utility_weight=1) #Negative fairness maybe

def collaborating(outcome: tuple[int, float, float, int], negotiation_scenario_perspective: ScenarioPerspective) -> float:
    return __calc_fairness(outcome, time_weight=-0.01, own_utility_weight=1, partner_utility_weight=1, joint_utility_weight=1, fairness_weight=1)

def compromising(outcome: tuple[int, float, float, int], negotiation_scenario_perspective: ScenarioPerspective) -> float:
    return __calc_fairness(outcome, time_weight=-0.075, own_utility_weight=0.25, partner_utility_weight=0.25, joint_utility_weight=0.5, fairness_weight=1)
...
def competing(outcome: tuple[int, float, float, int], negotiation_scenario_perspective: ScenarioPerspective) -> float:
    return __calc_fairness(outcome, time_weight=-0.025, own_utility_weight=1, fairness_weight=-1)
